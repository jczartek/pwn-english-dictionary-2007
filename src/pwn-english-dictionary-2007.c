...
#include "pwn-english-dictionary-2007-private.h"

#define SIZE_BUFFER  128
#define SIZE_ENTRY   64
#define DEFAULT_ENCODING "ISO8859-2"

struct _PwnEnglishDictionary2007
{
...
G_DEFINE_TYPE (PwnEnglishDictionary2007, pwn_english_dictionary2007, GY_TYPE_DICT)

PwnEnglishDictionary2007 *
pwn_english_dictionary2007_new (const gchar *id)
{
  return g_object_new (PWN_TYPE_ENGLISH_DICTIONARY2007, "identifier", id, NULL);
}

static GQuark
pwn_english_dictionary2007_error_quark (void)
{
  return g_quark_from_static_string ("pwn-english-dictionary-2007-error");
}


static void
pwn_english_dictionary2007_map (GyDict  *dict,
                                GError **err)
{
  ...
  GtkTreeIter iter;
  PwnEnglishDictionary2007 *self = PWN_ENGLISH_DICTIONARY2007 (dict);
...
static gchar *
pwn_english_dictionary2007_get_lexical_unit (GyDict  *dict,
                                             guint    index,
                                             GError **err)
{
  g_autoptr(GFileInputStream) in = NULL;
      ...
      if ((zerr = uncompress ((Bytef *)out_buffer, &destlen, (const Bytef *)in_buffer+i, MAXLEN)) != Z_OK)
        {
          switch (zerr)
            {
            case Z_BUF_ERROR:
              g_set_error_literal (err, pwn_english_dictionary2007_error_quark (), zerr,
...
static void
pwn_english_dictionary2007_finalize (GObject *object)
{
  PwnEnglishDictionary2007 *self = (PwnEnglishDictionary2007 *)object;

  if (self->file) g_clear_object (&self->file);

  if (self->offsets) g_clear_pointer (&self->offsets, g_free);

  G_OBJECT_CLASS (pwn_english_dictionary2007_parent_class)->finalize (object);
}
...
static void
pwn_english_dictionary2007_class_init (PwnEnglishDictionary2007Class *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GyDictClass *dict_class = GY_DICT_CLASS (klass);

  object_class->finalize = pwn_english_dictionary2007_finalize;

  dict_class->map = pwn_english_dictionary2007_map;
  dict_class->get_lexical_unit = pwn_english_dictionary2007_get_lexical_unit;
  dict_class->parse = pwn_english_dictionary2007_parse;
}

static void
pwn_english_dictionary2007_init (PwnEnglishDictionary2007 *self)
{
}
